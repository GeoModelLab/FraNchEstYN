...
epamig_data<-read.csv('brazil//data_epamig.csv') |>
...
weather_data<-read.csv('brazil//weather//SertaozinhoDaily.csv') |>
  mutate(lat =-18.5178,
         RAIN = RAIN)
...
thisCropParameters<-readRDS('brazil_wheat_parameters.rds')
...
for (pp in pps) {
  df<-FraNchEstYN::franchestyn(weather_data = weather_data,
                             ...
                             calibration="disease", #'all', 'crop', 'disease'
                             start_end = start_end,
                             iterations=999)
  ...
  all_parameters[[length(all_parameters) + 1]] <- out_df
...
pps <- unique(thisVariety$planting_period)

for (pp in pps) {
  thisPlantingPeriod <- thisVariety %>%
    filter(planting_period == pp)

  thisManagementData<- management_data |>
    filter(variety == v, planting_period == pp)
...
epamig_data_plot<-read.csv('brazil//data_epamig.csv') |>
  filter(cultivar=='BRILHANTE') |>
  group_by(year,sowing_date,cultivar,planting_period) |>
